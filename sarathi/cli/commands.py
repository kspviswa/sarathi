...
from sarathi import __version__, __logo__
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".sarathi" / "history" / "cli_history"
...
def onboard():
    ...
    from sarathi.config.loader import get_config_path, load_config, save_config
    from sarathi.config.schema import Config
    from sarathi.utils.helpers import get_workspace_path
    ...
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.sarathi/config.json[/cyan]")
...
def _create_workspace_templates(workspace: Path):
    ...
    templates_dir = pkg_files("sarathi") / "templates"
...
def _make_provider(config: Config):
    ...
    from sarathi.providers.custom_provider import CustomProvider
    ...
    from sarathi.providers.registry import find_by_name

    spec = find_by_name(provider_name)
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        console.print("[red]Error: No API key configured.[/red]")
        console.print("Set one in ~/.sarathi/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    from sarathi.config.loader import load_config, get_data_dir
    ...
    from sarathi.session.manager import SessionManager
    from sarathi.cron.service import CronService
    from sarathi.cron.types import CronJob
    from sarathi.heartbeat.service import HeartbeatService
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from sarathi.bus.events import OutboundMessage
...
def agent(
    ...
    if logs:
        logger.enable("sarathi")
    else:
        logger.disable("sarathi")
...
def channels_status():
    """Show channel status."""
    from sarathi.config.loader import load_config
...
def cron_list(
    ...
    """List scheduled jobs."""
    from sarathi.config.loader import get_data_dir
    from sarathi.cron.service import CronService
...
def cron_add(
    ...
    from sarathi.config.loader import get_data_dir
    from sarathi.cron.service import CronService
    from sarathi.cron.types import CronSchedule
...
def status():
    ...
    if config_path.exists():
        from sarathi.providers.registry import PROVIDERS
