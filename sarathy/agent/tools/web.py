...
def create_web_search_tool(provider: str, api_key: str | None = None, max_results: int = 5) -> Tool:
    """Factory function to create the appropriate web search tool based on provider."""
    if provider == "firecrawl":
        logger.debug("Web search provider: firecrawl")
        return FirecrawlSearchTool(api_key=api_key, max_results=max_results)
    else:
        logger.debug("Web search provider: brave")
